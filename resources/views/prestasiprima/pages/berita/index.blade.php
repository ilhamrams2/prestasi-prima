...
@section('content')
  <section class="bg-gray-50 relative z-10 pt-28 md:pt-36 pb-20">
    <div class="max-w-7xl mx-auto px-4 md:px-8">
      ...
      {{-- ===================================================== --}}
      {{-- BAGIAN ATAS: BERITA UTAMA (kiri) + HOT NEWS & AKSES CEPAT (kanan) --}}
      {{-- ===================================================== --}}
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div class="lg:col-span-2">
          ...
          {{-- FORM PENCARIAN --}}
          <form action="{{ route('berita.index') }}" method="GET" class="flex mb-8 shadow-md rounded-xl overflow-hidden"
            data-aos="fade-up">
          ...
          @if($news->count() > 0)
            ...
            <div class="mb-10 relative rounded-2xl overflow-hidden shadow-lg group" data-aos="zoom-in">
              <img src="{{ asset('storage/' . $first->thumbnail) }}" alt="{{ $first->title }}"
          ...
          @if($news->count() > 0)
            <div class="bg-white rounded-2xl shadow-md p-5" data-aos="fade-left">
              ...
              <a href="{{ route('berita.detail', $hot->slug) }}" class="block overflow-hidden rounded-xl">
                <img src="{{ asset('storage/' . $hot->thumbnail) }}" alt="{{ $hot->title }}"
      ...
      {{-- ===================================================== --}}
      {{-- BARIS KEDUA: 3 CARD LEBAR + 2 VIDEO (SELEBAR AKSES CEPAT) --}}
      {{-- ===================================================== --}}
        ...
        <div class="md:col-span-1 flex flex-col justify-end space-y-6">
          @foreach($videos->take(2) as $video)
            ...
            <div class="rounded-xl overflow-hidden shadow-md bg-white hover:shadow-lg transition">
              <iframe src="{{ $embedUrl }}" title="{{ $video->title }}" class="w-full h-48" allowfullscreen
                loading="lazy"></iframe>
              <div class="p-3">
      ...
      {{-- ===================================================== --}}
      {{-- BARIS KETIGA & KEEMPAT: CARD BERVARIASI (3 dan 2 kolom, full width) --}}
      {{-- ===================================================== --}}
      <div class="mt-20 space-y-16">
        ...
        <div class="grid md:grid-cols-2 gap-8">
          @foreach($news->skip(7)->take(2) as $item)
            <div
              class="bg-white rounded-2xl shadow-md hover:shadow-xl overflow-hidden transition transform hover:-translate-y-1 duration-300">
              <a href="{{ route('berita.detail', $item->slug) }}">
                <img src="{{ asset('storage/' . $item->thumbnail) }}" alt="{{ $item->title }}"
    ...
    {{-- ===================================================== --}}
    {{-- AJAKAN KE GALERI (PAKAI VIDEO DARI GALLERY) --}}
    {{-- ===================================================== --}}
    <section class="mt-24 bg-gradient-to-r from-orange-50 to-white py-16 relative overflow-hidden rounded-2xl">
      <div class="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-10">

        <!-- VIDEO dari Gallery -->
        <div class="w-full md:w-1/2" data-aos="fade-right">
          @php
            $galleryVideo = $videos->first();
            if ($galleryVideo) {
              $url = $galleryVideo->video_url;
              if (Str::contains($url, 'watch?v='))
                $videoId = Str::after($url, 'watch?v=');
              elseif (Str::contains($url, 'shorts/'))
                $videoId = Str::after($url, 'shorts/');
          ...
          @if(!empty($galleryVideo))
            <div class="rounded-2xl overflow-hidden shadow-lg bg-white">
              <iframe src="{{ $embedUrl }}" title="{{ $galleryVideo->title }}" class="w-full h-72 md:h-80" allowfullscreen
                loading="lazy"></iframe>
            </div>
          @else
            <img src="{{ asset('assets/images/section/gallery-preview.jpg') }}" alt="Galeri Prestasi Prima"
              class="rounded-2xl shadow-lg object-cover w-full h-72 md:h-80">
          @endif
        </div>
        ...
        <div class="w-full md:w-1/2" data-aos="fade-left">
          <h2 class="text-3xl font-extrabold text-gray-900 mb-4 leading-tight">
            Jelajahi <span class="text-orange-600">Galeri Prestasi Prima</span>
          </h2>
          <p class="text-gray-600 mb-6">
            Saksikan langsung berbagai kegiatan, perlombaan, dan momen berharga siswa kami
            melalui video dokumentasi yang penuh semangat dan inspirasi.
          </p>
          <a href="{{ route('gallery') }}"
            class="inline-block bg-orange-600 hover:bg-orange-500 text-white font-semibold px-6 py-3 rounded-lg shadow-md transition">
            Lihat Galeri →
          </a>
        </div>
      ...
      <!-- Ornamen Latar -->
      <div class="absolute -bottom-10 -right-10 w-64 h-64 bg-orange-100 rounded-full blur-3xl opacity-40"></div>
    </section>

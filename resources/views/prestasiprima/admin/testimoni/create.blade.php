...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-extrabold text-slate-800 tracking-tight">Input Testimoni</h1>
            <p class="text-sm text-slate-500 font-medium">Tambahkan testimoni baru dari siswa, alumni, atau stakeholder.</p>
        </div>
    </div>

    {{-- ================= FORM CARD ================= --}}
    <div class="bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('prestasiprima.admin.testimoni.store') }}" method="POST" enctype="multipart/form-data" class="divide-y divide-slate-50">
            @csrf
            ...
            <div class="p-8 space-y-8">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex flex-col md:flex-row items-center gap-6 p-6 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2rem] hover:border-orange-400 transition-colors group">
                        <div class="w-24 h-24 bg-white rounded-2xl flex-shrink-0 flex items-center justify-center shadow-sm overflow-hidden border border-slate-100 flex-col gap-1 relative p-2">
                            {{-- Icon Removed --}}
                        </div>
                        <div class="flex-1 text-center md:text-left">
                            <h4 class="text-sm font-bold text-slate-800 mb-1">Pilih Pas Foto</h4>
                            <p class="text-xs text-slate-500 font-medium mb-4 leading-relaxed">Format: JPG, PNG, WEBP (Maks. 1MB).</p>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="pesan" rows="5" 
                              ...
                              class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 focus:bg-white outline-none transition-all duration-300 font-medium text-slate-800" required>{{ old('pesan') }}</textarea>
                </div>
            </div>
